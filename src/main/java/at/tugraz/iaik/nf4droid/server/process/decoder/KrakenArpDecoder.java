...
public class KrakenArpDecoder extends ArpDecoder {
	...
	public void process(EthernetFrame frame) {
		...
		if (p.getHardwareSize() == 6 && p.getProtocolSize() == 4
				&& p.getHardwareType() == 1
			...
			try {
				...
				//fixing two times use of ArpProcessors with copy of ArpPacket where L2Frame is null
